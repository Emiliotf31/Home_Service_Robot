...
int main( int argc, char** argv )
{
  ...
  pick_obj.ns = "add_markers";
  pick_obj.id = 0;
  drop_obj.ns = "add_markers";
  drop_obj.id = 1;
  ...
  while(true){
    n.getParam("pick_up", pick_up);
    if (pick_up == true){
      ROS_INFO("The robot is picking up the object.");
  ...
  while(true){
    n.getParam("drop_off", drop_off);
    if (drop_off == true){
      ROS_INFO("The robot is droppig the object.");
      break;
    }
  }
